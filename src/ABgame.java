...
public class ABgame {
		...
		static int MinMax(char[] board,int depth)
		{
			...
			if(depth==0) {count_new++;
				count_min+=6;return static_estimation(board);}
			else 
		...
		{
			if(depth==0) {count_new++;count_min+=39;return static_estimation(board);}
			else 
			{	count_new++;
				count_min+=74;
				char[] n=new char[22];
		...
		public static void main(String args[]) throws Exception
		{
			...
			System.out.println("Positions Evaluated:"+count_new);
			out.write(bestmove);
			out.close();
