...
public class ABopening {
	...
	static int MaxMin(char[] board,int depth,int alpha,int beta,char player)
	{	count_min+=96;
		ArrayList<String> L=new ArrayList<String>();
		L=GenerateMovesOpening(board);
		int v=-10000;
		if(depth==0) return static_estimation(board);
		//if(player=='W')
			{
			for(int i=0;i<L.size();i++)
			
			{ 	
				v=Math.max(v,MinMax(L.get(i).toCharArray(),(depth),alpha,beta,'W'));
				if (v>=beta) {return v;}
				else {alpha=Math.max(v,alpha);bestmove=L.get(i);}
			}
			}
		return v;
	}
	static int MinMax(char[] board,int depth,int alpha,int beta,char player)
	{
		count_min+=10;
		ArrayList<String> L=new ArrayList<String>();
		L=GenerateMovesOpening(board);
		...
		//if(player=='W')
			...
			{ 	
				v=Math.min(v,MaxMin(L.get(i).toCharArray(),(depth-1),alpha,beta,'W'));
				if (v<=alpha) {return v;}
				else beta=Math.min(v,beta);
			}
			}
		return v;
	}
	...
	public static void main(String args[]) throws Exception
	{
		...
		int Min=MinMax(board,depth,-10000,10000,'W');
