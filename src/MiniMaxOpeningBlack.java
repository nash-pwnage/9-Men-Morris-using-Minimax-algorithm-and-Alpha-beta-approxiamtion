...
public class MiniMaxOpeningBlack {
	...
	static int MinMax(char[] board,int depth)
	{
		...
		if(depth==0) {count_min+=4766;return static_estimation(board);}
		else 
		{	count_min+=1922;
			int v = 100000; 
			ArrayList<String> L= new ArrayList<String>();
	...
	{
		if(depth==0) {count_min+=2599;return static_estimation(board);}
		else 
		{	count_min+=1274;
			char[] n=new char[22];
			int v = -100000; 
	...
	public static void main(String args[]) throws Exception
	{
		...
		// NEW ROUTINE TO CHANGE TO WHITE TO BLACK and REVERSE
		char[] board2=new char[22];
		for(int j=0;j<22;j++)
		{
			if(board[j]=='W') board2[j]='B';
			if(board[j]=='B') board2[j]='W';
			if(board[j]=='x') board2[j]='x';
		}
		
		int Min=MinMax(board2,depth);
		// convert it back !!
		char[] best=bestmove.toCharArray();
		char[] bestmove_update=new char[22];
		for(int j=0;j<22;j++)
		{
			if(best[j]=='W') bestmove_update[j]='B';
			if(best[j]=='B') bestmove_update[j]='W';
			if(best[j]=='x') bestmove_update[j]='x';
		}
		
		String bestmove2=new String(bestmove_update);
